package ds3code;
    ...
    public class Coordinator extends javax.swing.JFrame {
        ...
        int votes=0,ackvotes=0;
        static String msg="";  
        //initializing conditon variable for acktimer
        int flag=0;
        
        static Logger log_master = Logger.getLogger("Coordinator");  
        ...
        */
        private void requestVoteButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_requestVoteButtonActionPerformed
            ...
            try{
                ...
                jTextField1.setText("");
                log_master.info(chatInput +"WAIT");
                ...
                public void run() {
                    ...
                    try{
                     ...
                     {
                         try{
                             transferText(chatInput+" C_ACK");
                             log_master.info(chatInput +"PRE_COMMIT");
                             flag=1;
                             votes=0;
                             i=20;
                     ...
                     //if no of ack equals 3, which implies each participant has acknowledged
                     if(ackvotes==3)
                     {
                         try{
                    ...
                    if (i< 0)
                    {
                        if(flag==1){transferText(chatInput+" GLOBAL_COMMIT");
                        log_master.info(chatInput +"COMMIT");}
                        else{
                        transferText(chatInput+" GLOBAL_ABORT");
                        log_master.info(chatInput +"ABORT");
        ...
        public void transferText(String message) throws UnknownHostException, IOException {
            ...
            String chatInput = jTextField1.getText().trim();
            if(message.equals("logout")||message.contains("VOTE_REQUEST")||message.contains("C_ACK")||message.contains("GLOBAL_COMMIT")||message.contains("GLOBAL_ABORT"))
            {
                chatInput=message;
            }
